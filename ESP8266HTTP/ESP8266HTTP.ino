...
/*
Initialize the system by connecting to the network and starting various communications
*/
void setup() {
  pinMode(CS, OUTPUT);
  pinMode(2, OUTPUT);
...
/*
Main part of the program. Gets data from the server and sends it to the Due card.
*/
void loop() {

  if(WiFi.status() != WL_CONNECTED) { //check if we have disconnected from the network
    state = 0;
  }

  switch(state) {
    case 0: //in case we disconnect from the network
      newState = 2;
      digitalWrite(2, HIGH); //power off the connected indicator
    break;
    case 1: //get the objects from the server
      http.begin("http://192.168.20.133:5000/srv/objectlist");
      http.begin("http://192.168.20.12:5000/srv/objectlist");
      httpCode = http.GET();

      if(httpCode > 0){ //check if the request went OK
        ...
        sockArray[0] = 0x52; //agreed upon ID for the object
        ...
        delay(1);
        cubeArray[0] = 0x53; //agreed upon ID for the object
        ...
        delay(1);
        glassArray[0] = 0x54; //agreed upon ID for the object
      ...
      else { //if the request did not process, try again
        newState = 1;
      }
    break;
    case 2: //get the latest coordinates from the server
      ...
      if(httpCode > 0){
        ...
        led1Array[0] = 0x50; //agreed upon ID for LED 1
        ...
        delay(1);
        led2Array[0] = 0x51; //agreed upon ID for LED 2
      ...
      newState = 2; //keep getting the latest coordinates
    break;
  }
