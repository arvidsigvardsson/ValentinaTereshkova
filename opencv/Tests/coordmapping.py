...
class Mapper:
    ...
    def get_mapped_with_height_compensated(self, p, height):
        return get_weighted_compensation(self.get_mapped_with_height(p, height))
...
def get_weighted_compensation((x, y)):
    ...
    if (x + y) != 0:
        x_compensation = (xx_comp * x + xy_comp * y) / (x + y)
        y_compensation = (yx_comp * x + yy_comp * y) / (x + y)
    else:
        x_compensation = (xx_comp * x + xy_comp * y) / 2
        y_compensation = (yx_comp * x + yy_comp * y) / 2
